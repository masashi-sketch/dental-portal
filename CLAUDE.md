...
20. **商品マスタの実データ化（Shopify連携ロードマップPhase 1）**。患者ポータル`/shop`・`/shop/[id]`の静的12商品を廃止し、「BGJが商品マスタ管理（`/bgj/master/products`・`/api/bgj/products`）→ 各医院が表示ON/OFF（`/admin/products`を表示設定専用に全面書き換え・`/api/admin/product-settings`）→ 患者ポータルに反映（`/api/patient-portal/products`）」の実システムにした。新設テーブル`products`（先生のおすすめ4項目・詳細ページ4項目含む、公開/下書き）＋`clinic_product_settings`（**行なし=表示**のデフォルト表示設計。新商品は全医院に自動で並び、医院が非表示にした時だけ行をupsert）。患者用APIは意図的に一覧のみ（詳細ページも一覧からfind。非表示・非公開が直接URLでも漏れない）。詳細ページに残っていたEC要素（星評価・カート・数量・送料表記）も撤去し相談導線に統一。画像は`image_type`のCSS描画（共通`ProductVisual.tsx`）、色・カテゴリ候補は`src/lib/productDisplay.ts`に集約。`shop/data.ts`・`doctorContent.ts`は削除済み。**スコープ外（Phase 2以降）**：subscription系4画面・admin/orders/commission/dashboardの商品参照・Shopify同期・実画像（image_url列）・医院別先生コメント（clinic_recommendations）。事業モデルは「売主=医院、BGJは医院に卸すB2B」で確定（SalesforceはEnterprise Edition）。詳細はマニュアル`bgj/manual`のシステム手順「17. 商品マスタと医院ごとの表示設定」参照。
