...
27. **BGJ受注一覧からの代理受注・第一段階（2026-07-22）**。`/bgj/orders?view=received`の「＋ 新規受注」から、医院・その医院の有効な既存患者・医院で表示可能な公開商品（複数）を選び、医院受け取りの通常注文を直接登録する。PCは右サイドシート、モバイルは全画面シートで、閉じる操作は一覧へ遷移せずシートだけを閉じる。入力済みの場合は破棄確認を行う。登録APIはBGJ限定の`POST /api/bgj/orders`、候補取得は`GET /api/bgj/orders/create-options`。DB関数`create_bgj_patient_order`が所属・商品・数量を再検証し、ヘッダー・複数明細・`patient_order_events`監査履歴を1トランザクションで保存する。`created_via='bgj_portal'`で医院登録と識別し、`idempotency_key`で再送時の二重登録を防ぐ。DBは3NFを維持し、JSON明細はRPC入力にのみ使用して永続化しない。適用SQLは`supabase/migrations/20260722_bgj_direct_orders.sql`。第一段階の対象外は、新規患者の同時登録、自宅配送、決済、在庫確定、Shopify/Salesforce同期。
