...
4. 権限テーブルはこのプロジェクトに存在しないため対応不要。ただし**`src/app/bgj/manual/page.tsx`は存在する**（BGJポータル「マニュアル」、システム手順／利用マニュアルの2タブ構成）。DB・APIの新規変更や新機能を追加したときは、このページも一緒に更新すること（ページ冒頭にもその旨の注記あり）。カード等を縦に並べるのではなく、共通`SubTabs`コンポーネントでタブ切り替えにする方針（画面スクロールを減らすため）。マニュアルのナビゲーションツリー（利用マニュアル/システム手順の一覧）は`BgjSidebar.tsx`ではなく`src/app/bgj/manual/ManualNav.tsx`が持つ（詳細は「次のおすすめ」26番）。新しいマニュアル項目を追加する際は、`usageData.tsx`/`proceduresData.tsx`のデータ配列に加えて`ManualNav.tsx`の`manualNavItems`にもリンクを追加すること。**画像付きのStepsブロックを追加する場合、対応する画面のUIを変更したら`scripts/capture-manual-screenshots.mjs`で該当スクリーンショットを撮り直すこと**（撮り直さないと古い画面のまま`public/manual/`に残り続ける）。

# データベースを変更するときのルール

1. リレーショナルDBの設計は**第3正規形（3NF）を到達点**とする。第1正規形・第2正規形を満たしたうえで、非キー属性は主キーだけに依存させ、推移的関数従属を残さない。BCNF・第4正規形以降は、整合性上の明確な必要性と合意がある場合を除き自動的には適用しない。
   - 1つの事実は1か所で管理する。繰り返し項目は子テーブル、多対多関係は中間テーブル、医院・患者・商品などの主体はそれぞれのマスタへ分離する。
   - 画面表示の都合で、医院名・患者名・商品名などの**現在値**を別テーブルへ複製しない。外部キーとJOINで取得する。
   - 合計・件数・表示用ステータス名など、元データから導出できる値は原則として永続化せず、クエリ・RPC・VIEWで算出する。性能上の理由で保持する場合は、更新責任と整合性検証を明記する。
   - 注文明細の商品名・単価など、取引時点を保存する履歴スナップショットは、その注文／明細キーに依存する不変の事実として扱う場合に限り保持できる。現在のマスタ値の代替にせず、列名または設計文書でスナップショットであることを明示する。
   - 外部システムID・同期状態は、関係する集約ルートに1対1で属する場合だけ同じテーブルに置く。複数システムとの対応、同期試行履歴、複数値は連携用の子テーブルへ分離し、連番列や汎用JSONへ詰め込まない。
   - SQL作成前に、エンティティ、候補キー、外部キー、主な関数従属、一意制約、削除時の動作、必要なインデックスを確認し、既存スキーマへ重複する事実を追加しない。
2. `CREATE TABLE` / `ALTER TABLE`のSQLは、必ず以下をセットで提示する。
   ```sql
...
# 全画面の固定データ監査・レスポンス計測ロードマップ

2026-07-20に全54ページルートをコード上で予備監査し、今後は以下の方針を順番に消化することをユーザーが明示した。基本方針は、**固定データによる事実誤認を先に止め、既存のSentryを使って実測を開始し、測定結果に基づいて遅い箇所だけを改善する**こと。Shopify・Salesforce連携を待たずに進めるが、両サービスを代替する契約・決済・CRM機能は作り込まない。

## 固定データ監査の分類

全画面・全表示項目を次の4種類に分類する。

1. 今すぐDB/APIの実データへ置き換える業務データ
2. データ取得元や外部連携が未確定のため、取得できるまで非表示または「未集計」「未連携」「—」にする項目
3. ラベル、ステータス候補、説明文、マニュアル、メールプレビュー等の意図的な静的コンテンツ（許可リストに理由を記録して維持）
4. テスト・デモ専用データ（本番の業務事実としては表示しない）

`docs/fixed-data-audit.md`を監査台帳として作成し、ルート、権限、表示項目、データ取得元、書き込み先、分類、リスク、対応方針、確認テストを記録する。監査対象ページが増えた場合も同じ台帳へ追加する。

## 固定データ是正の優先順

### P0：誤認・疑似操作を先に除去

- BGJダッシュボード／レポートのKPI・売上推移・ランキング・期間表示等を実集計へ置き換える。完成までは「未集計」とし、架空の数字を表示しない。
- キャンペーン／Biogaia情報等、固定配列をローカルstateだけで追加・編集・削除し成功したように見せる操作を廃止する。永続APIが無い間は操作を無効化するか画面から除く。
- 患者ホーム／サプリ受け取り画面の患者名・次回予約・注文日・担当者等のプレースホルダーを、取得可能な実データへ置き換える。取得元が無い項目は非表示にする。
- `href="#"`、遷移しないリンク、処理のないボタンを全ポータルから除去し、実際の遷移先または処理がある場合だけ表示する。
- `/admin`の疑似ログインを廃止し、実際の認証画面または認証済みダッシュボードへ誘導する。
- サイドバーの固定未読数・固定日時等を廃止し、実データが無ければバッジを表示しない。

### P1：実データ化・所有範囲の確定

- BGJダッシュボード／レポートは`clinic_orders`、`clinic_visits`、`clinics`、`sales_reps`およびPostgres集計RPCをデータ源にし、CSV出力も表示中の実データから生成する。
- 営業担当変更履歴は監査ログを実装するか、実装まで履歴欄を撤去する。
- 医院の診療メニュー等は医院設定として永続化し、未設定時は表示しない。
- キャンペーンとBiogaia情報の正本・編集主体（BGJ全体か医院単位か）を決めてからスキーマを作る。決定前に保存できるようには見せない。
- 医院名・ブランド名・metadataのテスト名称を汎用名または実データへ置き換える。

### P2：再発防止と共通化

- 患者ポータルのHeader／Footer／ナビゲーションを共通化し、無効リンクや仮名称の重複を防ぐ。
- `scripts/audit-static-business-data.mjs`等のCI監査を追加し、固定された日付・金額・件数・患者名、`href="#"`、API書き込みを伴わない成功通知、ローカルstateだけの削除、既知の仮名称を検出する。
- 商品画像プレースホルダー、選択肢、メールプレビュー、マニュアル等の意図的な固定コンテンツは明示的な許可リストで除外する。
- 自動検出だけでなく、全主要動線を手動／E2Eでクリックし、handlerのないボタンや保存されない操作も確認する。

## 固定データ監査の完了条件

- 業務上のKPI、金額、注文、患者、医院、キャンペーン情報は、すべて台帳に記載したDB/API等の取得元を持つ。
- API書き込みが2xxで成功する前に成功通知を表示しない。
- `href="#"`やhandlerのない操作ボタンが本番画面に残っていない。
- データ表示領域にloading／empty／error状態があり、データが無い場合に架空値で埋めない。
- 維持する固定コンテンツは許可リストに理由がある。
- 既知の仮データを再導入するとCIまたはE2Eテストが失敗する。
...
- INP p75：200ms以内
- CLS p75：0.1以下
- 画面データ表示完了 p75：2.0秒以内、p95：3.5秒以内
- ウォーム時GET API p95：800ms以内
- 更新API p95：1.2秒以内
- DB query／RPC p95：300ms以内
- APIエラー率：1%未満

## 実施順と成果物

1. `docs/fixed-data-audit.md`とCI監査を作る。
2. P0の誤認表示・疑似操作を除去する。
3. Web VitalsとAPI／DB計測を追加する。
4. 全主要画面の性能基準値を測定し、`docs/performance-baseline.md`へ保存する。
5. BGJダッシュボード／レポート等のP1を実データ化する。
6. p75／p95が目標を外れた画面だけを優先して改善し、変更前後の数値を記録する。
7. Shopify／Salesforce接続時に未連携項目を各サービスの正本データへ置き換える。

成果物は、固定データ監査台帳、CI監査、性能基準レポート、Sentryダッシュボード／アラート、改善前後のスコアカードとする。概算は監査・P0で2〜3営業日、計測と基準測定で1〜2営業日、P1実データ化と初回性能改善で2〜3営業日（合計5〜8営業日）。各段階を小さく実装・検証・コミットして消化する。
...
14. 医院用ポータル「お知らせ管理」（`/admin/news`）は元々見た目だけのモック（ローカルstateのみ、DB未連携・患者ポータル未反映）だったが、`clinic_qa`（Q&amp;A機能）と同じ設計で実データ連携する機能として実装済み。新設テーブル`clinic_announcements`（`announcement_date`・`tag`・`text`・`status`。Q&amp;Aと異なり`sort_order`は持たず`announcement_date`降順で自動表示）、管理用API`/api/admin/clinic-announcements`（`resolveScopedCustomerCode`で医院は自院固定・BGJは代理編集）・患者ポータル向け公開read専用API`/api/patient-portal/announcements`（`status='公開'`のみ）、共有コンポーネント`src/components/ClinicAnnouncementManager.tsx`（医院用ポータルと得意先詳細「お知らせ」タブの両方から使用）という構成。患者ポータルのホーム画面はデスクトップ（既存のサイドバー内欄を実データ化）・モバイル（新規追加）の両方に表示し、取得失敗時は欄ごと非表示にしてホーム画面の他機能に影響しない設計。詳細はマニュアル`bgj/manual`のシステム手順「11. お知らせ機能」参照。
15. BGJポータルの使い勝手改善5点を実装済み：(a) 「マスタ &gt; 営業担当」をカード表示から一覧（テーブル）表示に変更し、Slack連携の設定有無をバッジで可視化、(b) 「マスタ &gt; 担当エリア」を自由入力から都道府県セレクトに変更（1エリア＝1都道府県。`src/lib/prefectures.ts`のPREFECTURES定数、`staff_areas.name`にCHECK制約追加。開発中のためデータ移行は未実施）、(c) `BgjSidebar.tsx`の「マスタ」「システム管理」配下を枠・背景で囲むグループ表示にし、見出しラベルの視認性も強化（`navGroups`でsectionLabelを起点に描画側だけグループ化。**当初は常時展開の枠のみで実装したが、視認性が伝わらないとのフィードバックを受け、後日（次項16番）折りたたみツリー構造に発展させた**）、(d) 新設「マスタ &gt; LINKマスタ」（新設テーブル`bgj_external_links`、`/api/bgj/external-links`。書き込みは`requireBgjSession`必須だがGETは任意の認証済みセッションに開放し、医院用ポータルの`AdminSidebar.tsx`から参照できるようにした）で、医院用ポータルのサイドバー「LINKS」欄（従来ハードコード3件）をBGJから追加・編集・削除できるようにした。医院用ポータル側は表示のみで、リンクごとの個別アイコンは廃止し全リンク共通の汎用アイコンに統一。詳細はマニュアル`bgj/manual`のシステム手順「12. BGJポータル使い勝手改善（マスタ一覧化・LINKマスタ）」参照。
16. BGJサイドバーのナビゲーションを**再帰的な折りたたみツリー構造**に発展させた。`BgjSidebar.tsx`の`ChildNavItem`型が自分自身を`children`に持てるようにし（何階層でも対応）、`ChildNavItemRow`という再帰コンポーネントで描画する。子項目を持つnavItemは「本体リンク＋開閉トグル」の構成になり、子は普段は折りたたまれて非表示、トグルクリック時または子孫のページを開いている時だけ表示される（`isHrefActive`/`hasActiveDescendant`でパス＋クエリパラメータを考慮したアクティブ判定を行う）。この仕組みを使い、「営業担当」配下に「役職マスタ」「担当エリア」を、「得意先一覧」配下に「ステータス」（17番参照）を折りたたみ子項目として配置した。また「マニュアル」配下も「利用マニュアル」（対象者別5項目）・「システム手順」（現在14項目、0〜13）の3階層ツリーにし、`/bgj/manual`ページ自体もURLクエリパラメータ（`?tab=usage&audience=...` / `?tab=procedure&step=...`）で表示内容を切り替える方式に全面書き換えた（ページ内のピル型タブ切り替えUIは撤去、QR自己登録配下の孫3項目のピルタブのみ維持）。697行だった`bgj/manual/page.tsx`は`manualComponents.tsx`（共通UI）・`proceduresData.tsx`（システム手順データ）・`usageData.tsx`（利用マニュアルデータ）に分割した。`useSearchParams`使用のため`BgjSidebar`・`bgj/manual/page.tsx`とも`Suspense`でラップしている。**今後navItemsに項目を追加する方針**：既存グループの末尾に`sectionLabel`を付けずに追加すれば自動的に同じ枠に入り、子を持たせたい場合は対象のnavItem（またはChildNavItem）に`children`配列を追加するだけでよい（`navGroups`のreduceロジック・`ChildNavItemRow`の再帰ロジックは変更不要）。また「得意先一覧」はサイドバー上「マスタ」枠の先頭（＝グループの起点、`営業担当`の直前）に配置されている（`sectionLabel`は「得意先一覧」側が持つ）。
17. 得意先（クリニック）の基本情報「ステータス」（活性/休眠/解約リスク）を、`clinics.status`のtext列＋CHECK制約によるハードコードから、営業担当マスタ（`sales_reps.role_id`→`staff_roles`）と同じFKパターンでBGJが追加・編集・削除できるマスタに変更した。新設`clinic_statuses`テーブル（`name`・`color`）を`clinics.status_id`で参照する（`on delete set null`、マスタ削除時は「未設定」に戻るだけで得意先データは消えない）。バッジ色もマスタに持たせ、Tailwindの動的クラス名生成を避けるため固定6色（emerald/amber/red/sky/violet/slate）のCHECK制約とし、`src/lib/clinicStatusColors.ts`で静的クラス名マップに変換する（ステータスマスタ管理画面・得意先一覧・得意先詳細の3箇所で共通利用）。API（`/api/bgj/clinic-statuses`）・管理画面（`/bgj/master/statuses`、サイドバーは「マスタ &gt; 得意先一覧」配下の折りたたみ子項目）は`staff-roles`/`bgj/master/roles`と同型。詳細はマニュアル`bgj/manual`のシステム手順「13. 得意先ステータスのマスタ化」参照。
18. ログイン画面の実装状況を棚卸しし、Moc（プロトタイプ）時代の名残による不具合を1件修正した。現状：患者様（`/`）・医院（`/clinic-login`）はID・パスワード、BGJ（`/bgj-login`）はGoogle OAuth（`@biogaia.jp`限定）という設計で、これはリリース前後を通じて変わらない最終形（ユーザーとの確認済み方針）。唯一`/auth/signin`（「ポータルを選択」画面、`src/app/auth/signin/page.tsx`）だけ、医院選択時は専用ログイン画面（`/clinic-login`）へ誘導するよう直っていたのに、**患者様選択時だけGoogle OAuthを試みる実装のまま残っていた**（開発初期に3ロールとも一旦Google認証で動作確認していた名残と推測される）。`src/auth.ts`のsignInコールバックはGoogle認証を`@biogaia.jp`ドメインに限定しているため、実際の患者様が押すと必ず失敗する不具合だった。医院と同じ扱い（Google OAuthを試みず`/`へ直接遷移）に修正済み。**新しいポータル・ロールを追加する際は、`/auth/signin`のPORTALS配列・`handleEnter`の分岐・`/bgj-login`のような専用ログイン画面すべてが実際のauth.ts提供元（Google OAuth or credentials）と一致しているか確認すること**（今回のように一部だけ更新漏れが起きやすい箇所）。
19. 患者様向けメールの実障害2件を解決し、医院スタッフにも同等のパスワードリセットを展開した。(a) **メール未達**：Google Workspace（`jyosys@biogaia.jp`）のアプリパスワード失効によるSMTP認証拒否（`535-5.7.8`）が原因だった。メール送信系の不具合報告はまず`vercel logs <本番URL> --expand --level error`でランタイムログを確認する（`console.error`はSentryでなくVercelログにのみ出る、TROUBLESHOOTING.md参照）。(b) **メール内リンクが開けない**：日本語本文のquoted-printableエンコードで76文字超の行にソフト改行が入り、トークン付き長URLが途中で壊れていた。`renderEmailTemplateHtml()`（`src/lib/email/templates.ts`）でURLを`<a href>`に埋め込んだHTML版を`sendPatientEmail`の`html`引数で併送して解決。**プレーンテキストのみで80文字超のURLを送るテンプレートを作らないこと**。(c) **医院スタッフのセルフサービスパスワードリセット**（`/clinic-forgot-password` → `/api/clinic-password-reset/request` → メール → `/clinic-reset-password` → `/api/clinic-password-reset/confirm`）を新設。`clinic_users.email`列（部分uniqueインデックス）＋`clinic_login_tokens`テーブル＋`src/lib/auth/clinicLoginToken.ts`（患者様用`loginToken.ts`のFK先違いの並行モジュール。2用途目のため意図的に共通化せず複製、3用途目が出たら共通化を検討）。メール文面は`clinic_email_templates`の対象外で固定文面のみ。BGJの得意先詳細「担当者」タブから担当者ごとにログインID・再設定用メール・状態・パスワードを管理する。`src/proxy.ts`の許可リスト追加も対応済み。手動・セルフサービスともパスワードは8〜128文字。詳細はマニュアル`bgj/manual`のシステム手順「16. 医院スタッフのパスワードリセット」参照。
20. **商品マスタの実データ化（Shopify連携ロードマップPhase 1）**。患者ポータル`/shop`・`/shop/[id]`の静的12商品を廃止し、「BGJが商品マスタ管理（`/bgj/master/products`・`/api/bgj/products`）→ 各医院が表示ON/OFF（`/admin/products`・`/api/admin/product-settings`）→ 患者ポータルに反映（`/api/patient-portal/products`）」の実システムにした。新設テーブル`products`＋`clinic_product_settings`（**行なし=表示**）。定期購入一覧・詳細も同じ患者用商品APIを使い、`subscription_available=true`だけを表示する。画像は`ProductVisual.tsx`、色・カテゴリ候補は`src/lib/productDisplay.ts`に集約。**引き続き外部連携待ち**：Shopify同期・実画像・決済・定期購入契約の確定/変更/解約、医院別先生コメント。事業モデルは「売主=医院、BGJは医院に卸すB2B」で確定（SalesforceはEnterprise Edition）。詳細はマニュアルのシステム手順「17」参照。
21. **Shopify・Salesforce連携前の開発方針と患者注文基盤**（2026-07-20にユーザーが明示）。最終形はShopify（商品・注文・定期購入）およびSalesforce（医院・患者等のCRM）連携とし、それまでに外部仕様へ依存しない画面・権限・内部API・テストを先行する。代替EC/CRMを作り込みすぎず、外部連携は交換可能なアダプターとして追加する。架空データや成功したように見えるローカル処理は残さない。この方針で`patient_orders`・`patient_order_items`を追加し、`source`/`external_order_id`/`sync_status`等で将来のShopify同期に備えた。医院用`/admin/orders`は固定データを廃止して実患者・公開商品から受け取り注文を登録・進捗更新し、患者用`/medication`は`/api/patient-portal/orders`から本人（または検証済みプレビュー患者）の注文だけを表示する。BGJ用`/bgj/orders?view=received`は`/api/bgj/orders`（BGJ限定・50件ページング）から全医院の患者注文を取得し、医院・患者番号・明細・金額・業務状態・外部同期状態を横断表示する。DB行は`src/lib/orderIntegration.ts`の`OrderIntegrationRecord`（`schemaVersion: 1`）へ変換し、画面と将来の外部アダプターがSupabase構造へ直接依存しないようにする。商品名・価格・用量・注意事項は注文時点のスナップショットを明細へ保存する。注文作成はDB関数`create_internal_patient_order`でヘッダー＋明細を1トランザクション化し、`idempotency_key`（医院＋キーの部分unique）で通信再送時の二重登録を防ぐ。画面は失敗時に同じキーを維持し、成功時だけ新しいキーへ切り替える。既存DBへの適用SQLは`supabase/migrations/20260720_patient_orders.sql`と`20260720_order_idempotency.sql`。**まだ実装しない範囲**：決済、在庫確定、定期課金、Shopify webhook、Salesforce同期。これらは接続仕様確定後に内部注文IDを維持したまま追加する。
22. **医院ダッシュボード・コミッション画面の固定データ廃止**。`get_admin_overview(customer_code)`で、有効患者数・公開中のお知らせ・対応中の患者注文・表示中の商品・最近の注文／お知らせ・直近6ヶ月の内部注文金額・今月の商品別内訳・契約コミッション率を1回のDB呼び出しで集計し、`/api/admin/overview`から自院スコープで返す。両画面は共有`useAdminOverview`でこの実データだけを表示する。内部注文の金額は決済・割引・税送料・返金を含まないため、必ず「参考注文金額」と表示し、`confirmedSales`・`confirmedCommission`はShopify決済連携が完了するまで`null`（画面は「—」）とする。架空の10%率、段階レート、目標達成率、月次グラフ、商品別販売数、患者来院数、保存されない経営情報フォームは削除した。既存DBへの適用SQLは`supabase/migrations/20260720_admin_overview.sql`。次段階はShopifyの契約・決済・返金webhookを同集計の確定値へ接続することで、内部注文の参考値と確定値を混同しない。
23. **2026-07-20時点の患者注文・集計基盤は本番反映済みで、ユーザーによる動作確認も成功**。注文／受け取り基盤はコミット`546627f`、ダッシュボード／コミッション実データ化はコミット`d9e8af9`。必要な3本のSQL（`20260720_patient_orders.sql`・`20260720_order_idempotency.sql`・`20260720_admin_overview.sql`）はSupabaseへ適用済みで、`get_admin_overview`が実環境から6ヶ月分を返すことも確認した。本番は`https://dental-portal-mock.vercel.app`、デプロイIDは`dpl_2zFKprUjv6Qp5TdZ7S5HqT8GG2Jw`（READY）。全体回帰は98ファイル・457テスト成功、TypeScript・本番ビルド成功。**今後の優先順と境界**：(1) Shopify接続前に`patient_subscriptions`等の読み取り用投影モデルと交換可能な`CommerceGateway`インターフェースを用意する、(2) Shopify仕様・利用アプリ確定後にselling plan／checkout／subscription contract／決済・返金webhook／変更・解約を接続する、(3) Salesforceは医院・患者・活動等のCRM正本として同期する。Shopifyを正本とする契約・決済・変更・解約をローカルだけで成立させたり、未接続処理を成功表示したりしない。ポータルDBは表示・検索・医院業務のための投影と同期状態を持つが、Shopify／Salesforceの代替システムにはしない。
24. **商品マスタに実画像アップロード機能を追加（Supabase Storage）**。手順17で`image_type`のグラデーション代替表示のみだった商品画像に、実ファイルアップロードを実装した。新設publicバケット`product-images`（5MB上限・jpeg/png/webp/gifのみ、バケット設定でも二重制限）、BGJ限定の新規API`/api/bgj/products/upload-image`（service_roleキーでアップロード・公開URL返却）、`products.image_url`列。このプロジェクトでSupabase Storageを使うのはこれが最初で、`storage.objects`にはRLSポリシーを一切定義せず、読み取りはpublicバケットのpublicエンドポイントで完結、書き込みは上記APIのみに限定する設計にした（DBの「RLS有効・ポリシーなし・service_roleキー経由のみ」の方針をStorageにも踏襲）。`ProductVisual.tsx`は`imageUrl`未設定時のみ従来のグラデーション表示にフォールバックする。既知の制約：画像差し替え・商品削除時の旧Storageオブジェクト自動削除は行わない（意図的なスコープ外）。詳細はマニュアルのシステム手順「19」参照。適用SQLはユーザーに提示済み（`alter table products add column image_url`＋`storage.buckets`へのbucket作成insert）。
25. **`/bgj/dashboard`・`/bgj/reports`の固定データ廃止（2026-07-20）**。`get_admin_overview`と同じ設計規約（`security definer`＋`revoke/grant`でservice_role限定）で新規RPC`get_bgj_dashboard_overview`・`get_bgj_sales_report`を追加し、`clinic_orders`・`clinic_visits`・`clinics`・`sales_reps`を集計する。適用SQLは`supabase/migrations/20260720_bgj_dashboard_reports.sql`。
    - **アラート閾値・レポート集計期間をBGJが自己管理できる設定にした**（架空の固定日数にしない、というユーザー方針）。`app_settings`に`dashboard_followup_days`（既定60）・`dashboard_dormant_days`（既定90、followup_daysより大きい必要がある制約付き）・`dashboard_include_never_ordered`（1件も注文が無い得意先を`contract_since`基準でアラート対象に含めるか）・`report_period_months`（既定6、1〜24の範囲）を追加し、`/bgj/system/settings`に編集セクションを新設した。API（`/api/bgj/dashboard-overview`・`/api/bgj/sales-report`）はこの設定値を読んでRPCへそのまま渡す。
    - 旧固定データにあった「30日で軽微アラート」の3段階目は、ユーザーが承認した2閾値（medium/high）に対応しない非設定項目のため廃止し、2段階＋「担当未割当」フラグ（日数条件と独立に必ず付与）に単純化した。
    - 月次売上推移の担当者別内訳は、担当者数が増えても破綻しないよう**直接描画する系列を上位4名（当該期間の合計値降順）までに制限し、残りは「その他」1本に合算する**設計にした（`dataviz` skillの検証済みカテゴリカル色は全ペア比較で4色までしか安全に判別できないため）。色の割り当ては表示のたびに順位で入れ替わらないよう、選ばれた上位4名の中では`staffId`の昇順で固定する（`src/app/bgj/dashboard/MonthlySalesChart.tsx`の`buildMonthlySalesChartData`、ロジックのみを`.test.ts`で単体テスト、実描画はrecharts個別テストの慣習に合わせて未実施）。
    - CSV出力（`/bgj/reports`、旧実装は`onClick`未接続のダミーボタンだった）は新規サーバーエンドポイントを作らず、`src/lib/csv.ts`（RFC4180エスケープ＋UTF-8 BOM付与）でクライアント側の取得済みJSONから生成する。現在選択中のタブ（月次推移／担当別／エリア別／上位得意先）のテーブルをそのままエクスポートする。
    - `docs/fixed-data-audit.md` #38・#39と横断コンポーネント欄（MonthlySalesChart/ReportsCharts）を対応済みに更新、`config/static-business-data-allowlist.json`の該当5エントリを削除した。詳細はマニュアルのシステム手順「20」参照。
26. **BGJマニュアルの3カラム化＋画像化（2026-07-21）**。ユーザーから「サイドバーがマニュアルのツリーに占領される」「画面全体を使い画像付きで詳細に」という2段階の要望を受けて全面改修した。
    - **ナビゲーション3カラム化：** `BgjSidebar.tsx`が内部に持っていた再帰ツリー描画ロジック（`ChildNavItem`型・`isHrefActive`・`hasActiveDescendant`・`NavItemRow`・`ChildNavItemRow`）を`src/app/bgj/components/navTree.tsx`に切り出し、`theme: 'dark' | 'light'`で配色を切り替えられるようにした（主サイドバーは`dark`、新設カラムは`light`）。「利用マニュアル」「システム手順」の26項目ツリーは`BgjSidebar.tsx`から削除し、新設`src/app/bgj/manual/ManualNav.tsx`（`navTree.tsx`を再利用）に移した。`src/app/bgj/manual/layout.tsx`（Next.js layout、`/bgj/manual`配下にのみ適用）が`<ManualNav />`と既存`page.tsx`を横並びにし、結果として「①主サイドバー（`w-60`、マニュアルは単純リンク）②マニュアル専用ツリー（`w-64`）③マニュアル詳細（`max-w-6xl`）」の3カラムになる。`BgjSidebar.test.tsx`の該当4テストは`src/app/bgj/manual/ManualNav.test.tsx`に移植した。
    - **画像化：** `manualComponents.tsx`の`Steps`に任意の`image?: { src, alt }`propを追加し、指定時だけ`grid md:grid-cols-2`で左テキスト・右画像の2カラムになる（未指定なら従来どおり1カラム）。システム手順の多くは`<Steps>`ではなく自由記述の`<p>`/`<Code>`のため、同じ2カラムパターンを任意コンテンツに適用できる`WithImage`ラッパーも追加した。画像は実際のアプリ画面のスクリーンショット（イラストではない）を使い、新設`scripts/capture-manual-screenshots.mjs`で撮影する。既存の確立済み手法（clinic-loginフォームログイン、`mintTestSession.mjs`のBGJセッション発行、`demo-patient-id`患者プレビュー）を流用し、対象画面ごとに`Card`要素単位で`locator.screenshot()`により部分切り取りする（fullPageは使わない）。同じ画面を複数のマニュアル項目から参照する場合はファイル名を使い回す。
    - **撮影スクリプトの既知の落とし穴：** (a) `page.goto(..., { waitUntil: 'networkidle' })`はdevサーバーのHMR再コンパイル・フォントpreload等で不安定にタイムアウトすることがあるため、`domcontentloaded`＋明示的な要素待ちに寄せた。(b) `domcontentloaded`直後はReactのhydrationが終わっておらず、その状態で`fill()`すると直後の再レンダリングで入力値が空文字に巻き戻る（controlled inputがhydration後にstate側の初期値へ揃うため）。ログインフォームはボタンが表示されるまで待ってから追加で待機し、hydration完了を確保してから入力する。(c) NextAuthのcredentialsサインインはdevサーバー負荷時に数秒かかることがあるため、固定待機ではなく`authjs.session-token`Cookieが実際に付与されるまでポーリングする。(d) recharts等`next/dynamic(ssr:false)`のチャートや複数API呼び出しをまとめて待つページは、初回表示が「読み込み中...」のままになりやすいため、該当ページだけ`waitAfterNav`を長め（6000〜9000ms）に設定する。
    - 対象範囲は既存の`<Steps>`ブロック1つ＝1画面操作という粒度に合わせ、実際にアプリ画面が存在する項目にのみ画像を付けた。Supabase/Google Cloud Console/Vercel管理画面・CLIコマンドのみのシステム手順（1・2・3・4・9）は対応する自社アプリ画面が無いため画像なしのまま。今後UIを変更した際は、対応する画像を`capture-manual-screenshots.mjs`で撮り直す運用が新たに必要になる。
27. **BGJ受注一覧からの代理受注・第一段階（2026-07-22）**。`/bgj/orders?view=received`の「＋ 新規受注」から、医院・その医院の有効な既存患者・医院で表示可能な公開商品（複数）を選び、通常注文を直接登録する。PCは右サイドシート、モバイルは全画面シートで、閉じる操作は一覧へ遷移せずシートだけを閉じる。入力済みの場合は破棄確認を行う。登録APIはBGJ限定の`POST /api/bgj/orders`、候補取得は`GET /api/bgj/orders/create-options`。`created_via='bgj_portal'`で医院登録と識別し、`idempotency_key`で再送時の二重登録を防ぐ。DBは3NFを維持し、JSON明細はRPC入力にのみ使用して永続化しない。適用SQLは`supabase/migrations/20260722_bgj_direct_orders.sql`。新規患者の同時登録、決済、在庫確定、Shopify/Salesforce同期は対象外。
28. **自宅受け取り（自宅配送）の復活（2026-07-22）**。医院用`/admin/orders`とBGJ用`/bgj/orders`の実注文で「医院で受け取り」「ご自宅へお届け」を選択できる。患者ポータルの定期購入シミュレーションでも自宅を選択し配送先を確認できるが、Shopify未接続のため契約・決済・注文完了は引き続き行わない。医院・BGJの登録処理は共通RPC`create_portal_patient_order`へ集約し、注文・複数明細・配送先・監査履歴を1トランザクション化した。初期適用SQLは`supabase/migrations/20260722_home_delivery.sql`。
29. **医院・患者の複数送り先と注文配送先履歴（2026-07-22）**。第3正規形の`delivery_destinations`で医院・患者それぞれが複数の送り先を所有できる。所有者は`clinic_customer_code`または`patient_id`の実FKどちらか一方に限定し、既定先は所有者ごとに最大1件とする。医院受け取りは注文医院の送り先、自宅配送は注文患者の送り先だけを選択可能で、共通RPCが所有者と有効状態を再検証する。注文時は`order_delivery_destinations`に選択元IDと住所スナップショットを1:1保存し、送り先変更後も過去注文を不変にする。送り先は物理削除禁止・`deleted_at`による論理削除とし、完了・キャンセル前の注文が参照中ならDBトリガーで削除を拒否する。医院・BGJの受注登録画面と患者ポータルで追加・選択・削除を行う。適用SQLは`supabase/migrations/20260722_delivery_destinations.sql`。DB変更時は同時更新必須のDB定義書（Excel・HTML・設計Markdown）にも反映済み。
30. **商品写真の全ポータル表示と医院価格（2026-07-22）**。`products.price`をBGJ基準価格、`clinic_terms.wholesale_rate`を契約仕切値率、`clinic_product_settings.clinic_price`を医院が変更できる患者表示価格とする。仕切値は`round(基準価格×仕切値率÷100)`で都度算出し、導出値を保存しないことで第3正規形を維持する。医院価格がNULLなら基準価格を使用する。患者商品API・医院/BGJ受注候補・注文作成RPCは医院価格へ統一し、注文明細には注文時点の医院価格と`image_url_snapshot`を保存する。商品写真は医院の商品管理、患者の商品一覧・詳細・定期購入・受け取り画面へ表示する。適用SQLは`supabase/migrations/20260722_clinic_product_pricing.sql`。DB定義書3形式も同時更新する。
31. **医院別定期購入価格と患者自宅配送先（2026-07-23）**。医院商品設定に3ヶ月・6ヶ月の月額を追加し、NULLの場合は医院通常価格へフォールバックする。同額の場合はNULLで保持し、既定値を重複保存しない。患者定期購入画面はハードコード割引を廃止して医院設定価格を表示する。自宅配送は患者の`delivery_destinations`から複数送り先を登録・選択する共通UIを使用する。適用SQLは`supabase/migrations/20260723_clinic_subscription_prices.sql`。DB変更時は生成元とDB定義書3形式を必ず同時更新する。
32. **医院商品管理の一括保存と単行表示（2026-07-23）**。価格の保存操作は明細行ごとに置かず、画面右上の「変更を保存」に集約する。編集された商品のみをまとめて保存する。商品一覧は列幅を固定して各明細を1行表示し、狭い画面では文字を折り返さず横スクロールで全列を維持する。
33. **定期購入正式ロジック・Shopify接続前Phase 1（2026-07-23）**。患者の申込を`patient_subscription_requests`・明細・配送先スナップショット・操作履歴の4テーブルへ3NFで保存し、`patient_orders`や将来の`patient_subscriptions`（Shopify契約投影）とは分離する。患者は3/6ヶ月、医院受け取り／自宅配送、複数送り先を選んで申込し、受付中のみ取消可能。BGJは`/bgj/orders?view=subscriptions`で全医院の申込を審査し、承認は「Shopify連携準備済み」を意味するだけで契約・決済・実受注を成立させない。登録は冪等、状態更新は楽観的排他、全操作は監査履歴へ保存する。受付中・承認済み申込が参照する送り先は論理削除不可。適用SQLは`supabase/migrations/20260723_patient_subscription_requests.sql`。Shopify接続後に承認済み申込を外部コマンドへ変換し、Shopifyから実注文を受信した時点で初めて`patient_orders`へ反映する。**2026-07-23に本番反映・受入テスト済み（詳細は#41）。**
34. **患者・医院ログイン背景の共通化（2026-07-23）**。患者ログインと医院ログインは、前回ログイン成功時に保存した共通Cookie`patient-last-clinic`の得意先コードから公開ブランディングAPIを呼び、`clinic_patient_settings.patient_background_url`と表示名を使用する。未ログイン状態で得意先を安全に判定できない初回アクセスだけ標準背景を表示する。Cookie保存・背景取得は`useLoginClinicBranding.ts`へ集約し、両ログイン画面で重複実装しない。
35. **定期購入Phase 2は後日実施（2026-07-23、ユーザー明示）**。承認済み定期購入申込をShopify連携へ渡す前段として、`commerce_accounts`、Outbox、交換可能な`CommerceGateway`、再送・エラー管理、BGJ連携状態表示、Webhook Inbox境界を実装する。Phase 2では実ストア送信・契約・課金・実受注・実Webhookを有効化せず、これらは接続情報と採用方式確定後のPhase 3で行う。ユーザーが再開を指示するまで着手しない。
36. **ウェビナー機能／Google Meet自動発行**。Phase 1は2026-07-23に本番反映・受入済み。2026-07-24拡張で、BGJの`/bgj/webinars`からGoogle Calendar APIへ開催予定を作成して固有Meet URLを自動取得できるようにした。認証はサービスアカウント＋Google Workspaceのドメイン全体委任を使い、開催者は`GOOGLE_CALENDAR_ORGANIZER_EMAIL`で固定する。必要環境変数は`GOOGLE_CALENDAR_SERVICE_ACCOUNT_EMAIL`・`GOOGLE_CALENDAR_SERVICE_ACCOUNT_PRIVATE_KEY`・`GOOGLE_CALENDAR_ORGANIZER_EMAIL`、許可スコープは`https://www.googleapis.com/auth/calendar.events`。Zoomは引き続き手動URL登録。対象医院の選択に加えて`webinar_target_contacts`でメール送付先担当者を個別保持し、医院単位・全医院一括選択と担当者単位の調整を提供する。各対象医院に有効かつメール登録済みの担当者1名以上をDB関数でも必須化し、公開時は選択担当者ごとにWorkspace SMTP＋`after()`で個別送信する。同じメールアドレスでも担当者が異なれば別メールを送る。Calendar予定IDは`webinar_sessions.external_space_id`に保持する。適用SQLは`supabase/migrations/20260724_google_meet_and_webinar_recipients.sql`。Zoom Server-to-Server OAuth、登録者同期・Webhook・出席・録画情報は後続対象。Zoom Meeting SDK埋め込みは対象外。
37. **医院業務連絡担当者（2026-07-23実装・DB適用成功済み）**。医院代表電話（`clinics.tel`）、医院認証・再設定用メール（`clinic_users`）、患者向けスタッフ紹介（`clinic_staff`）と分離し、`clinic_contacts`で医院ごとの複数担当者・主担当1人・有効/無効・論理削除・versionを管理する。2026-07-23の後続改修で、全担当者が`clinic_users`を必ず1つ持つ一対一へ変更した。連絡内容×方法は`clinic_contact_notification_preferences`、監査は`clinic_contact_events`へ3NFで分離する。BGJは得意先詳細「担当者」、医院は`/admin/clinic-info/contacts`から同じ共通UIで管理し、医院ロールは必ず自院スコープへ固定する。2026-07-24以降のウェビナー通知は開催ごとに`webinar_target_contacts`へ明示保存した担当者だけへ個別送信する。
38. **医院認証管理の担当者一覧集約（2026-07-23）**。BGJ得意先詳細の独立した「ログイン管理」タブを廃止し、得意先詳細「担当者」タブをカードではなく明細行で表示する。担当者登録と認証アカウント発行は一体化し、後付けの「医院ログインとの関連」項目は置かない。認証の正本は`clinic_users`で、`login_id`を画面上の担当者IDとして表示し、認証設定・パスワード初期化を担当者行から管理する。医院側も管理者に限り同じ更新経路を使用する（#40）。旧`ClinicLoginManager`は削除する。
39. **BGJ担当者横断一覧（2026-07-23）**。BGJサイドバーの「得意先一覧」直下に`/bgj/contacts`を配置し、全医院の`clinic_contacts`を得意先・役職・担当者名・連絡先で検索、得意先・役職・状態・担当内容（ウェビナー／受注・定期購入／請求／商品・キャンペーン／システム／営業）で絞り込む。ログインIDと有効状態も確認できる。横断画面は読み取り専用とし、更新は得意先詳細「担当者」へ集約して更新経路を増やさない。APIはBGJ限定の`GET /api/bgj/clinic-contacts`。**2026-07-23に本番反映・受入テスト済み（詳細は#41）。**
40. **医院担当者の個人ログイン・権限 Phase 1（2026-07-23）**。医院サイドバーの「クリニック情報」直下に「担当者管理一覧」を配置し、医院管理者とBGJが`clinic_contacts`の担当者単位で`clinic_users`の個人ログインを発行・更新する。権限・権限機能・ログイン権限割当は`clinic_portal_roles`・`clinic_portal_role_permissions`・`clinic_user_role_assignments`に3NFで分離。`admin`のみ担当者・ログイン管理可、`staff`は該当画面を閲覧のみ、`viewer`は医院ポータルの全更新APIをProxyで拒否する。担当者無効化・論理削除は関連ログインを自動無効化、パスワード変更・無効化は`session_version`を加算し旧JWTを即時失効。最後の有効な管理者は変更・無効化不可。適用SQLは`supabase/migrations/20260723_clinic_staff_access_phase1.sql`。DB定義書Excel・HTML・公開HTML、設計書、マニュアルを同時更新する。**2026-07-23に本番反映・受入テスト済み（詳細は#41）。**
41. **2026-07-23 本番反映・受入テスト完了**。`main`のコミット`3b0cd3e`をVercel本番`https://dental-portal-mock.vercel.app`へ反映した（デプロイID`dpl_Hajq8Bb3STb3MCC7Cx3cJ253pRWq`、`READY`）。反映前に179ファイル・845テスト、権限重点43テスト、ESLint、TypeScript、本番ビルド、固定業務データ監査がすべて成功。SupabaseではPhase 1の3権限、権限割当、`session_version`、`get_clinic_session_state`を確認した。本番受入では、医院管理者の主要画面・API、`staff`／`viewer`の閲覧と更新403、担当者無効化後の旧セッション即時失効、ウェビナーの下書き→公開→医院表示→通知処理→中止、定期購入の申請・冪等再送・受付中の患者取消・BGJ承認・承認後の患者取消拒否・BGJ取消を確認した。作成した担当者・ログイン・患者・ウェビナー・定期購入申請は削除済み。DB仕様で物理削除禁止のテスト用医院送り先1件だけをアーカイブ状態で残した。完了時点でVercel新規エラー、Sentry新規エラー、Git未コミット変更はいずれも無かった。**定期購入Phase 2の保留方針（#35）は変更せず、ユーザーが再開を指示するまで着手しない。**
42. 医院担当者の役職は`clinic_contact_roles`マスタ（医師・歯科医師・看護師・歯科衛生士・受付・その他）から選択し、`clinic_contacts.role_key`で参照する。担当者と`clinic_users`は必須の一対一で、担当者登録と認証アカウント発行を`create_clinic_contact_with_login`で同一トランザクションにする。`clinic_users.login_id`は画面上「担当者ID」と呼び、DBシーケンスで`A000001`から自動採番して医院ポータルへのログインに常に使用する。所属医院は両テーブルの`customer_code`を複合FKで一致させる。初期パスワード`A123456789A`と`must_change_password=true`で発行し、初回ログインは`/clinic-change-password`へ強制する。既存IDと従来のパスワード初期化は維持する。
43. 医院担当者のメールアドレスは`clinic_contacts.email`・`clinic_users.email`とも重複可とする。セルフサービスのパスワード再設定はメール単独検索では対象を特定できないため、`/clinic-forgot-password`で担当者IDとメールアドレスを入力し、両方が一致する有効な`clinic_users`だけにトークンを発行する。登録有無にかかわらず同じ成功レスポンスを返す探索対策は維持する。
