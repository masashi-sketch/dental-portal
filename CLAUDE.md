...
38. **医院ログイン管理の担当者一覧集約（2026-07-23）**。BGJ得意先詳細の独立した「ログイン管理」タブを廃止し、「担当者」タブの各担当者カードから医院ポータルのログインID発行・変更、パスワード再設定、再設定用メール、有効／無効を管理する。認証の正本は引き続き`clinic_users`で、`clinic_contacts.clinic_user_id`による関連だけを保持し、認証情報を重複保存しない。医院側の担当者画面にはログイン編集を表示しない。既存の医院ログインAPIを再利用し、旧`ClinicLoginManager`は削除する。**ユーザーの明示指示があるまでデプロイしない。**
