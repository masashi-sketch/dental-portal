...
10. 得意先ごとにカスタマイズできる患者様向けメール文面（初回登録メール・パスワード変更メール）の**編集・プレビュー画面のみ実装済み**（`clinic_email_templates`テーブル、BGJ得意先詳細「メール設定」タブ）。**実際のメール送信機能は未実装**（次点候補）。送信予定はGoogleWorkSpace（`jyosys@biogaia.jp`のアプリパスワード）経由の`nodemailer`で、新規の有料サービス契約は不要な設計。差出人は共通のエイリアス1つに固定し、`sender_name`列で得意先ごとに表示名だけ変える（実アドレスを得意先の数だけ用意するのは非現実的なため）。詳細はマニュアル`bgj/manual`のシステム手順「7. 患者様メール文面のカスタマイズ」参照。次点候補：(a) 実際の送信配線（nodemailer統合、環境変数追加）、(b) QR自己登録完了時の初回メール送信トリガー、(c) パスワード忘れ時の自己リセットフロー（メールリンクから直接ログイン完了させる、ワンクリックログイン用の新しいNextAuth Credentialsプロバイダが必要）、(d) 医院側での自己編集（現在はBGJ専用）
