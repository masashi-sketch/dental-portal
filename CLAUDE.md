...
36. **ウェビナー機能 Phase 1（2026-07-23実装・DB適用／実画面確認成功済み）**。BGJの独立メニュー`/bgj/webinars`でGoogle Meet／Zoomの開催情報を下書き作成・編集・公開・中止し、対象医院を選択する。医院は`/admin/webinars`で自院が対象の公開中開催だけを確認し、外部参加URLを新しい画面で開く。DBは`webinars`・`webinar_sessions`・`webinar_target_clinics`・`webinar_events`へ3NFで分離し、保存・状態遷移をDB関数でトランザクション化、versionで競合検出する。公開通知は対象医院の有効な医院ログインの登録メールへ既存Workspace SMTP＋`after()`で非同期送信する。Phase 1は外部API認証情報を持たず、実在する参加URLを手動登録する。次段階でGoogle Calendar/Meet APIとZoom Server-to-Server OAuth、その後に登録者同期・Webhook・出席・録画情報をAdapter経由で追加する。Zoom Meeting SDK埋め込みは初期対象外。適用SQLは`supabase/migrations/20260723_webinars_phase1.sql`で、2026-07-23にユーザーからDB適用と「下書き作成→公開→医院表示→参加URL」の実画面確認成功の報告あり。**ユーザーの明示指示があるまでデプロイしない。**
