...
36. **ウェビナー機能 Phase 1（2026-07-23実装・DB適用／実画面確認成功済み）**。BGJの独立メニュー`/bgj/webinars`でGoogle Meet／Zoomの開催情報を下書き作成・編集・公開・中止し、対象医院を選択する。医院は`/admin/webinars`で自院が対象の公開中開催だけを確認し、外部参加URLを新しい画面で開く。DBは`webinars`・`webinar_sessions`・`webinar_target_clinics`・`webinar_events`へ3NFで分離し、保存・状態遷移をDB関数でトランザクション化、versionで競合検出する。公開通知は医院業務連絡担当者の「ウェビナー・メール」を優先し、設定がない医院だけ有効な医院ログインメールへフォールバックして、既存Workspace SMTP＋`after()`で非同期送信する。Phase 1は外部API認証情報を持たず、実在する参加URLを手動登録する。次段階でGoogle Calendar/Meet APIとZoom Server-to-Server OAuth、その後に登録者同期・Webhook・出席・録画情報をAdapter経由で追加する。Zoom Meeting SDK埋め込みは初期対象外。適用SQLは`supabase/migrations/20260723_webinars_phase1.sql`で、2026-07-23にユーザーからDB適用と「下書き作成→公開→医院表示→参加URL」の実画面確認成功の報告あり。**2026-07-23に本番反映・通知を含む受入テスト済み（詳細は#41）。**
37. **医院業務連絡担当者（2026-07-23実装・DB適用成功済み）**。医院代表電話（`clinics.tel`）、医院認証・再設定用メール（`clinic_users`）、患者向けスタッフ紹介（`clinic_staff`）と分離し、`clinic_contacts`で医院ごとの複数担当者・主担当1人・有効/無効・論理削除・versionを管理する。2026-07-23の後続改修で、全担当者が`clinic_users`を必ず1つ持つ一対一へ変更した。連絡内容×方法は`clinic_contact_notification_preferences`、監査は`clinic_contact_events`へ3NFで分離する。BGJは得意先詳細「担当者」、医院は`/admin/clinic-info/contacts`から同じ共通UIで管理し、医院ロールは必ず自院スコープへ固定する。ウェビナー通知は`get_webinar_notification_recipients`で新設定を優先し、その医院にウェビナーメール担当者がいない場合だけ有効な`clinic_users.email`へフォールバックする。
