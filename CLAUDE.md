...
28. **自宅受け取り（自宅配送）の復活（2026-07-22）**。医院用`/admin/orders`とBGJ用`/bgj/orders`の実注文で「医院で受け取り」「ご自宅へお届け」を選択できる。自宅配送では郵便番号・都道府県・市区町村・番地・任意の建物名・受取人名・電話番号を必須検証し、`patient_orders`へ重複保存せず、注文に1:1で従属する`order_shipping_addresses`へ注文時点の配送先スナップショットとして保存する。患者ポータルの定期購入シミュレーションでも自宅を選択し配送先を確認できるが、Shopify未接続のため契約・決済・注文完了は引き続き行わない。医院・BGJの登録処理は共通RPC`create_portal_patient_order`へ集約し、注文・複数明細・配送先・監査履歴を1トランザクション化した。患者ポータルの受け取り画面、医院一覧、BGJ一覧では自宅配送注文の配送先を表示する。適用SQLは`supabase/migrations/20260722_home_delivery.sql`。
