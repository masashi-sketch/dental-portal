...
37. **医院業務連絡担当者（2026-07-23実装・DB適用成功済み）**。医院代表電話（`clinics.tel`）、医院ログイン・再設定用メール（`clinic_users`）、患者向けスタッフ紹介（`clinic_staff`）と分離し、`clinic_contacts`で医院ごとの複数担当者・主担当1人・任意の医院ログイン関連・有効/無効・論理削除・versionを管理する。連絡内容×方法は`clinic_contact_notification_preferences`、監査は`clinic_contact_events`へ3NFで分離する。BGJは得意先詳細「担当者」、医院は`/admin/clinic-info/contacts`から同じ共通UIで管理し、医院ロールは必ず自院スコープへ固定する。ウェビナー通知は`get_webinar_notification_recipients`で新設定を優先し、その医院にウェビナーメール担当者がいない場合だけ有効な`clinic_users.email`へフォールバックする。適用SQLは`supabase/migrations/20260723_clinic_contacts.sql`。トリガーを`DROP TRIGGER IF EXISTS`後に再作成する再実行可能な構成へ修正し、2026-07-23にユーザーからSupabase適用成功の報告あり。DB定義書・設計書・マニュアルも同時更新済み。**2026-07-23に本番反映・受入テスト済み（詳細は#41）。**
38. **医院ログイン管理の担当者一覧集約（2026-07-23）**。BGJ得意先詳細の独立した「ログイン管理」タブを廃止し、「担当者」タブの各担当者カードから医院ポータルのログインID発行・変更、パスワード再設定、再設定用メール、有効／無効を管理する。認証の正本は引き続き`clinic_users`で、`clinic_contacts.clinic_user_id`による関連だけを保持し、認証情報を重複保存しない。その後のPhase 1で、医院側も管理者に限り同じ担当者単位の更新経路を使用する方針へ拡張した（#40）。旧`ClinicLoginManager`は削除する。**2026-07-23に本番反映・受入テスト済み（詳細は#41）。**
