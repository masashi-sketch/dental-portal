...
医院ポータルの認証情報は引き続き`clinic_users`を正本とし、`clinic_contacts`へログインIDやパスワードを重複保存しない。BGJ得意先詳細の「担当者」タブから、担当者ごとにログインIDの発行・変更、パスワード再設定、有効／無効、再設定用メールを管理する。医院側の担当者画面には認証情報の編集権限を与えない。
