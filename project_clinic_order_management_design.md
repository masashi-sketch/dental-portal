...
担当者の役職は自由入力を廃止し、`clinic_contact_roles`の「医師、歯科医師、看護師、歯科衛生士、受付、その他」から選択する。`clinic_contacts.role_key`を外部キーとし、既存の未定義役職は「その他」へ移行する。

新規担当者IDはDBシーケンスにより`A000001`から全体連番で自動採番し、既存IDは変更しない。`clinic_users.customer_code`と`clinic_contacts.customer_code`の一致は複合外部キーで保証し、担当者IDから所属クリニックを一意に識別する。初期パスワードは`A123456789A`で発行し、`clinic_users.must_change_password`により初回ログイン直後の変更を必須とする。変更完了時はセッション世代を更新して再ログインさせる。メールによるセルフリセットと管理画面からの既存パスワード変更は従来どおり利用できる。

同じメールアドレスは複数担当者で使用できる。セルフサービスのパスワード再設定はメールアドレスだけで検索せず、「担当者ID＋メールアドレス」の組み合わせで対象認証アカウントを一意に特定する。

権限は`clinic_portal_roles`、許可機能は`clinic_portal_role_permissions`、ログインへの割当は`clinic_user_role_assignments`に分離する。`admin`は担当者・ログインを管理でき、`staff`は医院業務を行い担当者管理は閲覧のみ、`viewer`は医院ポータル全体の更新系APIを利用できない。担当者の無効化・論理削除時は関連ログインも自動無効化する。`clinic_users.session_version`をパスワード変更・無効化時に加算し、発行済みJWTを即時失効させる。医院ごとの最後の有効な`admin`は変更・無効化を禁止する。

BGJポータルの`/bgj/contacts`は全医院の担当者を横断する読み取り専用一覧とし、得意先、役職、担当者名、状態、担当内容で絞り込む。編集は得意先詳細の担当者タブへ集約し、一覧画面に別系統の更新処理を持たせない。
