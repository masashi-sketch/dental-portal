...
## 表示完了の直接計測（2026-07-22追加）

全画面ローディング導入後は、`data-app-ready="true"`になった時刻を`displayReadyMs`として直接記録する。従来の`wallMs`（`networkidle`到達時刻）も比較用に残すが、性能目標との比較にはユーザーが実際に画面を操作できる`displayReadyMs`を使用する。

低速通信時の表示制御は`npm run test:e2e:loading`で検証する。`/api/auth/session`を1.5秒遅延させ、応答前は全画面待機が維持され、応答後に本文が一括表示されることを実ブラウザで確認する。また、通信が終了しない場合も15秒で本文を表示するフェイルセーフを設けている。

このE2Eは対象サーバーが未起動ならローカルdevサーバーを自動起動・終了する。GitHub ActionsのCIにもChromiumのセットアップとテスト実行を組み込み、push・pull requestごとに表示制御の回帰を検出する。

### 導入後の実測結果（2026-07-22）

ローカルdevサーバーに対して各画面ウォームアップ2回＋10回計測。単位はms。`displayReadyMs`は全画面ローディングが消え、本文を操作できる状態になった時刻を表す。
