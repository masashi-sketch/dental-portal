...
create unique index if not exists clinic_contacts_clinic_user on public.clinic_contacts(clinic_user_id)
...
create or replace function public.save_clinic_contact(
  ...
  p_name text, p_department text, p_title text, p_email text, p_phone text,
...
begin
  ...
  if p_contact_id is null then
    insert into public.clinic_contacts(customer_code, clinic_user_id, name, department, title, email, phone, is_primary, status, notes)
    values (p_customer_code, p_clinic_user_id, trim(p_name), nullif(trim(p_department), ''), nullif(trim(p_title), ''),
    ...
    update public.clinic_contacts set clinic_user_id = p_clinic_user_id, name = trim(p_name), department = nullif(trim(p_department), ''),
      title = nullif(trim(p_title), ''), email = lower(nullif(trim(p_email), '')), phone = nullif(trim(p_phone), ''),
...
create or replace function public.save_clinic_contact_login(p_contact_id uuid,p_customer_code text,p_login_id text,p_password_hash text,p_email text,p_status text,p_role_key text,p_actor_type text,p_actor_identifier text)
returns uuid language plpgsql security definer set search_path=public as $$
...
begin
 if nullif(trim(p_login_id),'') is null or char_length(trim(p_login_id)) not between 3 and 100 or trim(p_login_id)~'[[:space:]]' or p_status not in ('有効','無効') or p_role_key not in ('admin','staff','viewer') or p_actor_type not in ('bgj','clinic','system') then raise exception 'ログイン入力が不正です'; end if;
 ...
 if v_user_id is null then
  ...
  insert into public.clinic_users(customer_code,login_id,password_hash,name,email,status,password_changed_at) values(p_customer_code,trim(p_login_id),p_password_hash,v_contact_name,lower(nullif(trim(p_email),'')),p_status,now()) returning id into v_user_id;
...
declare v_id uuid; begin if nullif(p_password_hash,'') is null then raise exception 'パスワードが必要です'; end if; update public.clinic_users set password_hash=p_password_hash,password_changed_at=now(),session_version=session_version+1,failed_login_attempts=0,locked_until=null where id=p_clinic_user_id returning id into v_id; if v_id is null then raise exception '医院ログインが見つかりません'; end if; return v_id; end; $$;
drop function if exists public.get_clinic_session_state(uuid);
create function public.get_clinic_session_state(p_clinic_user_id uuid) returns table(status text,session_version integer,role_key text,permissions text[]) language sql stable security definer set search_path=public as $$
 select u.status,u.session_version,coalesce(r.role_key,'admin'),array(select p.permission_key from public.clinic_portal_role_permissions p where p.role_key=coalesce(r.role_key,'admin') order by p.permission_key) from public.clinic_users u left join public.clinic_user_role_assignments r on r.clinic_user_id=u.id where u.id=p_clinic_user_id;
$$;
