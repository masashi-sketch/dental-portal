  ...
  const save = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/clinic-contacts/${contact.id}/login`, {
        ...
        body: JSON.stringify({ password: form.password || undefined, email: form.email, status: form.status, roleKey: form.roleKey }),
      });
      const body = await response.json().catch(() => null);
      if (!response.ok) throw new Error(body?.error ?? 'ログイン情報を保存できませんでした。');
      const savedUser = body.clinicUser as ClinicUserWithRole;
  ...
  return <div className="fixed inset-0 z-[60]" role="dialog" aria-modal="true" aria-labelledby="clinic-login-editor-title">
    ...
    <section className="absolute inset-0 flex min-h-0 flex-col bg-white shadow-2xl sm:left-auto sm:w-[min(560px,94vw)]">
      ...
      <div className="min-h-0 flex-1 space-y-5 overflow-y-auto p-5">
        ...
        <label className="block"><span className="text-sm font-bold text-slate-700">担当者ID</span><input readOnly value={clinicUser.login_id} className="mt-2 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-3 font-mono text-slate-600" /></label>
        <label className="block"><span className="text-sm font-bold text-slate-700">新しいパスワード（変更時のみ）</span><input type="password" autoComplete="new-password" value={form.password} maxLength={128} onChange={(event) => setForm({ ...form, password: event.target.value })} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-3 focus:ring-2 focus:ring-violet-400" /><span className="mt-1 block text-xs text-slate-400">8〜128文字。既存のパスワード初期化として利用できます。</span></label>
        ...
        <label className="block"><span className="text-sm font-bold text-slate-700">ログイン状態</span><select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as LoginForm['status'] })} className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3 py-3"><option value="有効">有効</option><option value="無効">無効</option></select></label>
        <div className="rounded-xl bg-slate-50 px-4 py-3 text-xs leading-relaxed text-slate-500">担当者IDは発行後に変更できません。このIDで医院ポータルへログインします。業務連絡用メールとパスワード再設定用メールは用途を分けて管理します。</div>
      </div>
      <footer className="flex justify-end gap-2 border-t border-slate-200 px-5 py-4"><button type="button" onClick={() => !saving && onClose()} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-600">キャンセル</button><button type="button" disabled={saving} onClick={() => void save()} className="rounded-xl bg-violet-600 px-5 py-2.5 text-sm font-bold text-white hover:bg-violet-500 disabled:opacity-50">{saving ? '保存中…' : '認証設定を保存'}</button></footer>
    </section>
  </div>;
}
