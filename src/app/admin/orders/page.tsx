          ...
          <Card theme="sky" className="p-5 shadow-sm">
            ...
            <form onSubmit={createOrder} className="grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
              <label className="text-sm text-slate-600">患者
                <select value={patientId} onChange={(event) => setPatientId(event.target.value)} required className="mt-1 w-full rounded-xl border border-sky-200 bg-white px-3 py-2.5 text-slate-800">
              ...
              <label className="text-sm text-slate-600">受け取り方法
                <select value={fulfillmentMethod} onChange={(event) => setFulfillmentMethod(event.target.value as FulfillmentMethod)} className="mt-1 w-full rounded-xl border border-sky-200 bg-white px-3 py-2.5 text-slate-800">
