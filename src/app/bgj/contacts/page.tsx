...
import type { BgjClinicContactListItem, ClinicContactTopic } from '@/lib/supabase/types';
...
export default function BgjContactsPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/bgj/clinic-contacts', { signal: controller.signal })
      .then(async (response) => {
        ...
        setContacts(body.contacts ?? []); setError(null);
      })
      .catch((loadError) => {
  ...
  const titles = useMemo(() => [...new Set(contacts.map((contact) => contact.title).filter((value): value is string => !!value))].sort((a, b) => a.localeCompare(b, 'ja')), [contacts]);
  ...
  const filtered = useMemo(() => {
    ...
    return contacts.filter((contact) => {
      const searchable = [contact.customer_code, contact.clinic?.name, contact.name, contact.department, contact.title, contact.email, contact.phone, contact.clinic_user?.login_id].filter(Boolean).join(' ').toLowerCase();
      return (!query || searchable.includes(query))
        && (customerCode === 'all' || contact.customer_code === customerCode)
        && (title === 'all' || contact.title === title)
        && (status === 'all' || contact.status === status)
        && (topic === 'all' || enabledTopics(contact).includes(topic));
    });
  }, [contacts, customerCode, search, status, title, topic]);
  ...
  return <div className="p-4 sm:p-6">
    ...
    <Card className="mb-4 p-4">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <select aria-label="役職で絞り込み" value={title} onChange={(event) => setTitle(event.target.value)} className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm"><option value="all">すべての役職</option>{titles.map((option) => <option key={option} value={option}>{option}</option>)}</select>
      ...
      <div className="overflow-x-auto"><table className="min-w-[1050px] w-full"><thead><tr className="border-b border-slate-200 bg-slate-50 text-left text-xs font-semibold text-slate-500"><th className="px-4 py-3">得意先</th><th className="px-4 py-3">役職</th><th className="px-4 py-3">担当者名</th><th className="px-4 py-3">担当内容</th><th className="px-4 py-3">メール</th><th className="px-4 py-3">電話番号</th><th className="px-4 py-3">ログインID</th></tr></thead><tbody>
        {filtered.map((contact) => {
          ...
          return <tr key={contact.id} className="border-b border-slate-100 align-top transition-colors hover:bg-violet-50/40">
            ...
            <td className="px-4 py-4 text-sm text-slate-600"><p>{contact.title ?? '—'}</p>{contact.department && <p className="mt-0.5 text-xs text-slate-400">{contact.department}</p>}</td>
            ...
            <td className="max-w-[220px] break-all px-4 py-4 text-xs text-slate-600">{contact.email ?? '—'}</td><td className="whitespace-nowrap px-4 py-4 text-xs text-slate-600">{contact.phone ?? '—'}</td><td className="px-4 py-4 text-xs"><p className="font-mono text-slate-600">{contact.clinic_user?.login_id ?? '未発行'}</p>{contact.clinic_user && <p className={`mt-0.5 font-semibold ${contact.clinic_user.status === '有効' ? 'text-emerald-600' : 'text-slate-400'}`}>{contact.clinic_user.status}</p>}</td>
          </tr>;
        })}
