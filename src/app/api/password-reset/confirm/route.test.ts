...
const consumeLoginTokenMock = vi.fn(async (): Promise<{ patientId: string } | null> => null);
vi.mock('@/lib/auth/loginToken', () => ({
  consumeLoginToken: (...args: unknown[]) => consumeLoginTokenMock(...(args as [])),
}));

const { POST } = await import('./route');

function confirmRequest(body: Record<string, unknown>) {
  return new NextRequest('http://localhost/api/password-reset/confirm', {
    method: 'POST',
...
describe('POST /api/password-reset/confirm', () => {
  beforeEach(() => {
    updateSpy.mockClear();
    consumeLoginTokenMock.mockClear();
    consumeLoginTokenMock.mockResolvedValue(null);
  });
  ...
  it('パスワードが8文字未満なら400（トークン検証前に弾く）', async () => {
    ...
    expect(res.status).toBe(400);
    expect(consumeLoginTokenMock).not.toHaveBeenCalled();
  });
  ...
  it('有効なトークンならパスワードをハッシュ化して更新する', async () => {
    consumeLoginTokenMock.mockResolvedValue({ patientId: 'patient-1' });
