...
CURRENT_TABLES = [
    ...
    TableDefinition("現行", "ウェビナー", "webinars", "実装済み",
    ...
    TableDefinition("現行", "ウェビナー開催枠", "webinar_sessions", "実装済み・Google Meet連携",
        "開催日時、Google Meet／Zoomと参加URLを本体から分離する。", [
            ...
            c("外部スペースID", "external_space_id", "text", nullable="YES", description="Google Calendar予定ID"),
            c("作成日時", "created_at", "timestamptz", default="now()"), c("更新日時", "updated_at", "timestamptz", default="now()"),
        ]),
    TableDefinition("現行", "ウェビナー対象医院", "webinar_target_clinics", "実装済み",
    ...
    TableDefinition("現行", "ウェビナー送付先担当者", "webinar_target_contacts", "実装済み",
        "公開メールを送る担当者をウェビナーごとに個別保持する多対多中間表。", [
            c("ウェビナーID", "webinar_id", "uuid", "PK / FK", constraint="webinars.id ON DELETE CASCADE"),
            c("担当者ID", "contact_id", "uuid", "PK / FK", constraint="clinic_contacts.id ON DELETE RESTRICT"),
            c("作成日時", "created_at", "timestamptz", default="now()"),
        ]),
    TableDefinition("現行", "ウェビナー操作履歴", "webinar_events", "実装済み",
        "作成・編集・公開・中止を監査保存する。", [
            c("イベントID", "id", "uuid", "PK", default="gen_random_uuid()"),
    ...
    TableDefinition("現行", "医院担当者役職", "clinic_contact_roles", "実装済み",
        "医院担当者の役職を選択肢として一元管理する。", [
            c("役職キー", "role_key", "text", "PK", constraint="physician / dentist / nurse / dental_hygienist / receptionist / other"),
            c("役職名", "label", "text", "UK"), c("表示順", "sort_order", "integer", "UK"),
            c("作成日時", "created_at", "timestamptz", default="now()"), c("更新日時", "updated_at", "timestamptz", default="now()"),
        ]),
    TableDefinition("現行", "医院業務連絡担当者", "clinic_contacts", "実装済み",
        "医院ごとに複数の業務連絡担当者と主担当、論理削除、排他制御を管理する。", [
            ...
            c("認証アカウント内部ID", "clinic_user_id", "uuid", "UK / FK", constraint="clinic_users.(id, customer_code) ON DELETE RESTRICT"),
            c("氏名", "name", "text"), c("部署", "department", "text", nullable="YES"),
            c("役職キー", "role_key", "text", "FK", default="other", constraint="clinic_contact_roles.role_key ON DELETE RESTRICT"),
    ...
    TableDefinition("現行", "医院ポータルログイン", "clinic_users", "実装済み・Phase 1",
        "担当者ID・認証情報・所属得意先・セッション世代を保持する。全担当者が1対1でclinic_contactsを持つ。", [
            ...
            c("担当者ID兼ログインID", "login_id", "text", "UK", description="A+6桁。画面では担当者IDとして表示し、この値でログインする"), c("パスワードハッシュ", "password_hash", "text"),
            ...
            c("初回変更必須", "must_change_password", "boolean", default="false", description="新規発行時true。初回変更または既存リセット完了時false"),
        ]),
...
EXACT_SCHEMA_TABLES = {
    ...
    "webinar_target_contacts",
    "webinar_events",
    "clinic_contacts",
    "clinic_contact_roles",
...
RELATIONSHIPS_CURRENT = [
    ...
    ("ウェビナー", "webinars.id", "1", "N", "webinar_target_contacts.webinar_id", "ウェビナー送付先担当者", "PK・FK・CASCADE"),
    ("医院業務連絡担当者", "clinic_contacts.id", "1", "N", "webinar_target_contacts.contact_id", "ウェビナー送付先担当者", "PK・FK・RESTRICT"),
    ...
    ("医院担当者役職", "clinic_contact_roles.role_key", "1", "N", "clinic_contacts.role_key", "医院業務連絡担当者", "FK・RESTRICT"),
    ("医院認証アカウント", "clinic_users.(id, customer_code)", "1", "1", "clinic_contacts.(clinic_user_id, customer_code)", "医院業務連絡担当者", "UK・複合FK・RESTRICT"),
...
CONSTRAINTS = [
    ...
    ("現行", "UK10", "webinar_target_contacts", "webinar_id, contact_id", "PRIMARY KEY", "同一ウェビナーの送付先担当者重複防止。"),
    ("現行", "IDX7", "webinar_target_contacts", "contact_id, webinar_id", "INDEX", "担当者から送付対象ウェビナーを検索。"),
    ("現行", "UK11", "clinic_contacts", "customer_code WHERE is_primary AND status='active' AND deleted_at IS NULL", "PARTIAL UNIQUE", "医院ごとの有効な主担当を1人に限定。"),
    ("現行", "UK12", "clinic_contacts", "clinic_user_id", "UNIQUE", "1担当者と1認証アカウントを必須の一対一にする。"),
    ("現行", "UK13", "clinic_users", "login_id", "UNIQUE", "担当者ID兼ログインIDの重複防止。"),
